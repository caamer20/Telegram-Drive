...
use crate::{NativePlaybackState, NativePlayerExt, NativePlayerResult, NativePlayerSource, Result};
...
pub(crate) async fn take_pending_native_player_restore<R: Runtime>(
    ...
    app.native_player().take_pending_restore().await
}

#[command]
pub(crate) async fn clear_pending_native_player_restore<R: Runtime>(
    app: AppHandle<R>,
) -> Result<()> {
    app.native_player().clear_pending_restore().await
}
